  ...
  >
    ...
    >
      ...
      <button
        ...
        class="absolute bottom-4 right-4 px-3 py-1.5 bg-black bg-opacity-60 text-white text-xs rounded-lg backdrop-blur-sm hover:bg-opacity-80 transition-all"
      >
        1:1
      </button>
    </div>
    ...
    >
      ...
      <p class="text-sm text-gray-500">Lade Bild…</p>
    </div>
    ...
    >
      ...
      <div class="text-center">
        <h3 class="text-lg font-medium text-gray-300">
          {{ camera ? camera.name : 'Unbekannte Kamera' }}
        </h3>
        <p class="text-sm max-w-xs px-4">
          Bitte konfiguriere eine Snapshot-URL in den Einstellungen.
        </p>
      </div>
    </div>
  </div>
...
// Preloader: swap image only when fully loaded
...
const onPreloaded = () => {
  displayUrl.value = preloadUrl.value;
